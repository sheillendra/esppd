...
class InitialDataForm extends Model {
    ...
    const TABLE_NAME = [
        ...
        'pegawai' => [
            'id' => '', 'opd_id' => '', 'nama_tanpa_gelar' => '', 'nip' => '',
            'pangkat_golongan_id' => '', 'eselon_id' => '', 'status' => '',
            'gelar_depan' => '', 'gelar_belakang' => ''
        ],
        'pejabat_daerah' => [
            'jabatan_daerah_id' => '', 'penduduk_id' => '',
            'tanggal_mulai' => '', 'dasar_hukum' => '', 'status' => ''
        ],
        'pejabat_struktural' => [
            'id' => '', 'jabatan_struktural_id' => '', 'pegawai_id' => '',
            'status' => '',
        ],
        'pejabat_keuangan' => [
        ...
        'penduduk' => [
            'id' => '', 'nik' => '', 'nama_tanpa_gelar' => '', 'jenis_kelamin' => '',
    ...
    public function upload() {
        try {
            ...
            foreach (self::TABLE_NAME as $tableName => $header) {
    ...
    protected function columnChecker(&$header, $sheet) {
        $isComplete = true;
        foreach (self::COL_NAME as $colName) {
        ...
        for ($i = 2; $i <= $highestRow; $i++) {
            ...
            foreach ($header as $colAttr => $colName) {
                ...
                $model->{$colAttr} = trim($sheet->getCell($colName . $i)
                                ->getValue());
                $model->{$colAttr} = $model->{$colAttr} === '' ? null : $model->{$colAttr};
            }
            if ($model->save()) {
                if (isset($header['id'])) {
