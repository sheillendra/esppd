window.yii.app = (($) => {
    ...
    var ajaxMap = {
        pangkat: (callback) => {
            return refAjax(callback, 'pangkat', { r: 'v1/pangkat-golongan', fields: 'kode, pangkat_lengkap' }, 'kode', 'pangkat_lengkap');
        },
        eselon: (callback) => {
            return refAjax(callback, 'eselon', { r: 'v1/eselon', fields: 'kode, eselon' }, 'kode', 'eselon');
        },
        opd: (callback) => {
            return refAjax(callback, 'opd', { r: 'v1/opd', fields: 'id, nama' });
        },
        jabatanStruktural: (callback) => {
            return refAjax(callback, 'jabatanStruktural', { r: 'v1/jabatan-struktural', fields: 'id, nama' });
        },
        jabatanDaerah: (callback) => {
            return refAjax(callback, 'jabatanDaerah', { r: 'v1/jabatan-daerah', fields: 'id, nama' });
        },
        kategoriBiayaSppd: (callback) => {
            return refAjax(callback, 'kategoriBiayaSppd', { r: 'v1/kategori-biaya-sppd', fields: 'id, nama', expand: 'jenisBiayaSppds' });
        },
        jenisBiayaSppd: (callback) => {
            return refAjax(callback, 'jenisBiayaSppd', { r: 'v1/jenis-biaya-sppd', fields: 'id, nama' });
        },
        satuan: (callback) => {
            return refAjax(callback, 'satuan', { r: 'v1/satuan', fields: 'id, nama' });
        },
        wilayah: (callback) => {
            return refAjax(callback, 'wilayah', { r: 'v1/wilayah', fields: 'kode, nama, kode_induk', sort: 'urutan' }, 'kode', 'nama');
        },
    };
    ...
    var createTreeData = (res) => {
        ...
        $.each(res, (k, v) => {
            ...
            if (v.kode_induk === null) {
                yii.app.ref.wilayahTreeList.push(item);
                mapItems[v.kode] = yii.app.ref.wilayahTreeList[yii.app.ref.wilayahTreeList.length - 1];
            } else {
    ...
    var wilayahLoader = (params, success, error) => {
        if (yii.app.ref.wilayahTreeList) {
            /** JSON parse dan stringify ada cara clone array nested */
            success(JSON.parse(JSON.stringify(yii.app.ref.wilayahTreeList)));
        } else {
            if (yii.app.refResult['wilayah']) {
                createTreeData(yii.app.refResult['wilayah']);
                success(JSON.parse(JSON.stringify(yii.app.ref.wilayahTreeList)));
            } else {
                if (ajaxOnProcess['wilayah']) {
                    ajaxQueueCallback['wilayah'].push(() => {
                        success(JSON.parse(JSON.stringify(yii.app.ref.wilayahTreeList)));
                    });
                } else {
                    ajaxMap.wilayah((refData, res) => {
                        createTreeData(res);
                        success(JSON.parse(JSON.stringify(yii.app.ref.wilayahTreeList)));
                    });
                }
    ...
    return {
        ...
        ref: {
            ...
            statusPelaksanaTugas: { 1: 'Belum SPPD', 2: 'Sudah SPPD' },
            statusSuratTugas: { 1: 'Masih diproses', 2: 'Menuggu TTD', 3: 'Sudah Terbit' },
            statusSppd: { 1: 'Sedang  proses', 11: 'Hitung Biaya', 20: 'Pengesahan', 30: 'Terbit', 35: 'Dibatalkan', 40: 'Hitung rampung' },
            levelWilayah: { 1: 'Desa', 2: 'Kecamatan', 3: 'Kabupaten/Kota', 4: 'Provinsi', 5: 'Negara' },
            kategoriWilayah: { 0: '', 1: 'Dalam Daerah', 2: 'Luar Daerah Dalam Provinsi', 3: 'Luar Daerah Luar Propinsi', 4: 'Luar Negeri' },
            jenisBiayaSppdByKategori: {},
        },
        refResult: {},
        refResultById: {},
        ...
        jabatanStrukturalDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/jabatan-struktural',
                    fields: 'id, nama, nama_2, singkatan, opd_id',
                    expand: 'opd',
                }),
                method: 'get',
                mode: 'remote',
                ...
                tipPosition: 'top',
                formatter: (row) => {
                    return '<span style="font-weight:bold">' + row.nama_2 +
                        '</span><div style="font-style: italic;font-size:11px;line-height:11px">' + row.nama +
                        '</div><span style="font-size:11px">' + row.opd.nama + '</span>';
                },
            }, options));
        },
        pegawaiDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/pegawai',
        ...
        pendudukDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/penduduk',
        ...
        produkHukumDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/produk-hukum',
        ...
        pejabatDaerahDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/pejabat-daerah',
                    fields: 'id, penduduk.nama_tanpa_gelar, jabatanDaerah.nama',
                    expand: 'penduduk, jabatanDaerah'
                }),
                method: 'get',
                mode: 'remote',
                valueField: 'id',
                ...
                labelPosition: 'top',
                formatter: function (row) {
                    var s = '<span style="font-weight:bold">' + row.penduduk.nama_tanpa_gelar + '</span><br/>' +
                        '<span style="color:#888">' + row.jabatanDaerah.nama + '</span>';
                    return s;
                },
                loadFilter: function (data) {
                    return $.map(data, function (v) {
                        v.pejabat = v.penduduk.nama_tanpa_gelar;
                        return v;
                    });
                }
            }, options));
        ...
        pejabatStrukturalDropdown: function (element, options = {}) {
            return element.combobox($.extend({
                url: yii.easyui.getHost('api'),
                queryParams: yii.easyui.ajaxAuthToken({
                    r: 'v1/pejabat-struktural',
                    fields: 'id, pegawai.nama_tanpa_gelar, jabatanDaerah.nama',
                    expand: 'pegawai, jabatanStruktural'
                }),
                method: 'get',
                mode: 'remote',
                valueField: 'id',
                ...
                labelPosition: 'top',
                formatter: function (row) {
                    var s = '<span style="font-weight:bold">' + row.pegawai.nama_tanpa_gelar + '</span><br/>' +
                        '<span style="color:#888">' + row.jabatanStruktural.nama + '</span>';
                    return s;
                },
                loadFilter: function (data) {
                    return $.map(data, function (v) {
                        v.pejabat = v.pegawai.nama_tanpa_gelar;
                        return v;
                    });
                }
            }, options));
        ...
        anggaranDropdown: (element, options = {}, callback = function () { }) => {
            return element.combobox($.extend(
        ...
        wilayahDropdown: function (element, options = {}, callback = function () { }) {
